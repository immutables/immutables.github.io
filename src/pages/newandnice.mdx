...
_Note: source access workaround, when Immutables tries to look at the source code instead of
relying on language elements and types model, works on `nullableAnnotation` configured, like  `@Nullable`.
If you use fully-qualified name, it will NOT match! The parser and matcher are not quite smart there._
