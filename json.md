...
We _do not use_ [ObjectMapper](https://github.com/FasterXML/jackson-databind) — the bean/POJO mapping facilities. _Immutables_ provides an alternative bindings based on straightforward code generation.
However, immutables could be integrated in Jackson POJO/Object mapper, enabling you to use _Immutables_' objects as part of POJO and vice-versa. See API documentation for annotation @[Jackson.Mapped](https://github.com/immutables/immutables/blob/master/value/src/org/immutables/value/Jackson.java).
...
Compile and runtime utilities used during marshaling can be easily added as one repackaged
[org.immutables:json-runtime:{{v}}]({{ depUri }}|json-runtime|{{ v }}|jar) jar. This jar contains
only json related classes from `common` artifact and required _Jackson_ libraries as transitive dependencies (`jackson-core`, `jackson-databind`, `jackson-annotations`).
...
- `IOException`s for input output errors and JSON syntax problems, missing attributes
- Generated entries in `/META-INF/services/*` may end up corrupted or missing due to jar repackaging or custom broken build processes, this may cause problems while locating marshalers for types.
